...
%let specialvisit='计划外访视' '安全性随访1' '安全性随访2' '安全性随访3' '肿瘤进展随访' '生存随访';
...
proc sql;
create table crfmiss1 as 
select b.vari,a.* from crfmiss a
left join derived.rand b on a.subjid=b.subjid
;
quit;

data edc.crfmiss;
set crfmiss1;
if vari eq '二药联合' and dmname in ('三药联合PK/PD-SHR-9146给药记录' '三药联合PK采血' '三药联合PD采血' '三药联合全血诱导试验' ) then delete ;
if vari eq '三药联合' and dmname in ('二药联合PK/PD-SHR9146给药记录' '二药联合PK采血' '二药联合PD采血' '二药联合全血诱导试验' ) then delete ;
