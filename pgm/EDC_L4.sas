...
%let specialvisit='生存随访','退出研究治疗','妊娠报告';
...
set  sub_uncollect;
	...
	else if  &visdat. ne '' and visitname ^ in (&specialvisit.) then  output prefinal_1;
run;
...
data prefinal_1;
set prefinal_1;
if visitname in('末次治疗后30天','末次治疗后60天  ','末次治疗后90天  ')  then visitid=put(input(visitid,best.)-1,4.);
run;
data  sv_1;
set  sv_1;
if visitid  in('40','41','42')  then visitid=put(input(visitid,best.)-1,4.);
run;
