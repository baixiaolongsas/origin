...
data lastdat;
	set sv_last_ds1_ds;
	lastdat=min(input(dthdat,yymmdd10.),input(dsdat,yymmdd10.));
...
data prefinal_2_2;
	set prefinal_2_1;
/*(where=(lastdat>close or lastdat=.));*/
...
data rand;
set derived.rand(keep= subjid randnext randdat);
if randnext ne '';
proc sort data=rand;
by subjid;
run;
...
data prefinal_2_3;
merge prefinal_2_2(in=a) rand;
by subjid;
if a;
dif =today()-input(randdat,YYMMDD10.);
if visitname eq '透析前随访期' and (randnext eq '透析后随访(HD V0)' or dif lt 84) then delete;
label dif='系统日期-随机日期';
run;
...
data edc.visitmiss;
	...
	set prefinal_2_3 prefinal_1_1;
	if ^missing(open)  then 
	day=today()-open;
	else day=.;
	visitnum=input(visitid,best.);
	keep studyid siteid subjid status visitname visitnum visitid open close day randnext randdat dif;
	label day ='访视缺失距今天数';
run;
