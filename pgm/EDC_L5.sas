...
%let specialvisit='生存随访','C8后访视','计划外访视','妊娠记录';
%let ds1=ds1;/*治疗结束页名称，是否有项目会有多个治疗结束页，需要确定用哪个*/
%let novisdat='共同页','研究治疗结束/退出研究'; /*有的项目不统计计划外*/
...
data sv_last;
	set sv;
	if compress(&visit.)='研究总结';
	label &visdat.='研究总结访视日期';
	keep subjid &visdat.;
run;
/**************************************无ds1 dth表  ds表无dsdat变量（下）*********************************************************/
...
/*	by subjid;*/
/*run;*/
data sv_last_ds1_ds; set sv_last;run;
/**************************************无ds1 dth表  ds表无dsdat变量（下）*********************************************************/
...
proc sort data=prefinal_2_1_1 ;by subjid visitnum;run;
data prefinal_2_2;
	set prefinal_2_1_1(where=(lastdat>close or lastdat=.));
	visdat__=lag(&visdat.);
	by subjid visitnum;
	if first.subjid then visdat__='';
	if visitname='筛选期(D-7~D1)' and (visdat_ ne '' or (close ne . and  today()-close >=15)) and crfnum1 = . then output;
	if visitname ne '筛选期(D-7~D1)' and visdat__ ne '' and (visdat_ ne '' or (close ne . and  today()-close >=15)) and crfnum1 = . then output;
run;
...
/*	if &visdat. ne '' then visdat_=lag(&visdat.);*/
	...
	by subjid descending visitnum;
	if first.subjid then visdat_='';
	if (visdat__ ne '' or (close ne . and  today()-close >=15)) and crfnum1 = .;
run;
