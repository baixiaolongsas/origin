...
%let novisdat='共同页','治疗结束页'; /*有的项目不统计计划外*/
...
data sv_last;
	set sv;
	if compress(&visit.)='提前出组/研究结束(W52D364)';
...
data ds1(rename=( statdat=statdat1));
	set derived.ds1;
	keep subjid statdat;
run;
proc sort;by subjid;run;
...
data lastdat;
	set sv_last_ds1_ds;
	lastdat=min(input(statdat1,yymmdd10.),input(&visdat.,yymmdd10.),input(statdat,yymmdd10.));
	format lastdat yymmdd10.;
	label lastdat='最小退出/给药日期';
	keep subjid lastdat statdat;
run;
...
data prefinal_1 prefinal_2;
	...
	else if input(visitid,best.) in(1:20) then output  prefinal_2;
run;

data prefinal_1_1;
	set prefinal_1;
	if statdat ne "" and crfnum1=.;
/*	这里需要ds不为空，因此改成ds中的研究中止/完成日期不为空*/
run;
...
/*	if &visdat. ne '' then visdat_=lag(&visdat.);*/
	...
	if (visdat_ ne '' or (close ne . and  today()-close >=5)) and crfnum1 = .;
run;
