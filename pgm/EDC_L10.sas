...
proc sort data=selected out=selected(where=(lockstat='未冻结'));by pub_rid;run;
...
proc sql;
	create table zsview1 as select siteid,count(pub_rid) as zs1 '未冻结页数' from prefinal group by siteid;
	create table EDC.zsview21 as select a.*,coalesce(left(compress(put(b.zs1,best.),'.')),'0') as zs1 '未冻结页数' from zsview as a left join zsview1 as b on a.siteid=b.siteid;
quit;
