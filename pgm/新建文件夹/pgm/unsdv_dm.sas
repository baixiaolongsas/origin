...
proc sql;
	create table hchzb_sum as select jl,yhczdsle 'CRA已核查字段数量',f15 'CRA需核查字段数量' from dp.hchzb where f15>yhczdsle;
quit;
...
data final;
	retain COL6 COL7 COL8 COL2 COL5   COL9 COL10 COL11 yhczdsle f15 COL14 COL16 COL17 COL18 WARNING ;
	set final_final;
...
DATA abs_final;
	...
	keep tid siteid subjid lockstat sitename   visit visitnum svnum yhczdsle f15 creator createtime modify modifytime WARNING x length;
run;
proc sql;
...
/*sum(f15)-sum(yhczdsle) as unsdv,*/
/*sum(yhczdsle) as  yhczdsle,*/
/*sum(f15) as  f15 */
/*from dp.unsdv_cra*/
/*group  by siteid*/
/*order by siteid*/
