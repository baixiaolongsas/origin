...
options fmtsearch=(dc);
proc sql;
	create table hchzb_sum as select jl,yhczdsls 'CRA已核查字段数量',f16 'CRA需核查字段数量' from dp.hchzb where f16>yhczdsls;
quit;
 proc format library=dc cntlout=work.cntlfmt;quit;
 proc sort data=cntlfmt(keep=FMTNAME LENGTH) nodupkeys out=fmt;by _all_;run;
%macro formatall;
data vstable;
	set sashelp.vstable(where=(libname='DC' ));
	N=_N_;
	call symput('n',max(N));
...
%do i=1 %to &n.;
	...
	quit;
	proc contents data=dc.&name. out=_label_&name.(keep=NAME FORMAT INFORMAT LENGTH VARNUM) noprint ;quit;
	proc sql;
...
data final;
	retain COL6 COL7 COL8 COL2 COL5   COL9 COL10 COL11 yhczdsls f16 COL14 COL16 COL17 COL18 WARNING ;
	set final_final;
...
DATA abs_final;
	...
	keep tid siteid subjid lockstat sitename   visit visitnum svnum yhczdsls f16 creator createtime modify modifytime WARNING x length;
run;
proc sql;
	select max(length)+1 into:cut from abs_final;
quit;
%put &cut.;
...
data dp.unsdv_med;
	set abs_final(where=(lockstat ne '未提交'));
	if visit ne '';
	drop x length WARNING creator createtime modify modifytime ;
run;
/**/
/*proc sql;*/
/*create table atest as*/
/*select siteid as siteid,*/
/*sum(f16)-sum(yhczdsls) as unsdv,*/
/*sum(yhczdsls) as  yhczdsls,*/
/*sum(f16) as  f16 */
/*from dp.unsdv_cra*/
/*group  by siteid*/
/*order by siteid*/
