...
/*无icfdat变量*/
proc sql;
	create table prefinal as select a.*,b.sn,c.dat,status from selected as a 
...
data EDC.unsub;
	retain studyid siteid subjid status visit pub_tname    sn  dat lockstat lastmodifytime;
	set prefinal;
	keep studyid siteid subjid status  visit pub_tname    sn  dat lockstat lastmodifytime;
